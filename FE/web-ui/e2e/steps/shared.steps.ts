  ...
  async init(requiresAuth: boolean = false, authState?: any): Promise<void> {
    ...
    const contextOptions =
      requiresAuth && authState
        ? { storageState: authState, viewport: { width: 1600, height: 768 } }
        : {};
...
When(
  "I click the {string} testId button",
  async function (this: CustomWorld, buttonId: string) {
    await this.page.getByTestId(buttonId).click({ timeout: 45000 });
  },
);

When(
  "I click the {string} checkbox",
  async function (this: CustomWorld, checkboxName: string) {
    await this.page
...
When(
  ...
  async function (this: CustomWorld, inputString: string, fieldName: string) {
...
When(
  "I enter {string} into the {string} testId field",
  async function (this: CustomWorld, inputString: string, fieldId: string) {
...
When(
  "I change {string} status to {string}",
  async function (this: CustomWorld, name: string, newStatus: string) {
...
Then(
  "I should see the text {string} in {string} color",
  async function (
    this: CustomWorld,
    expectedText: string,
    expectedColor: string,
  ) {
    const textLocator = await this.page.getByText(expectedText, {
      exact: true,
    });
...
Then(
  "I should see the headline {string}",
...
Then(
  ...
  async function (this: CustomWorld, textContent: string) {
    await expect(this.page.getByText(textContent).first()).toBeVisible({
      timeout: 45000,
    });
  },
);
Then(
  "I should not see the {string} text",
  async function (this: CustomWorld, textContent: string) {
    await expect(this.page.getByText(textContent).first()).toBeHidden({
      timeout: 45000,
    });
  },
);
...
Then(
  "I should see the following components:",
  async function (this: CustomWorld, dataTable: DataTable) {
    const expected = dataTable.raw().flat();
    for (const componentId of expected) {
      await expect(this.page.getByTestId(componentId).first()).toBeVisible({
        timeout: 45000,
      });
    }
  },
...
Then(
  ...
  async function (this: CustomWorld, pageName: string) {
    ...
    await expect(this.page).toHaveURL(expectedPath, { timeout: 45000 });
...
Then(
  ...
  async function (this: CustomWorld, expectedButton) {
